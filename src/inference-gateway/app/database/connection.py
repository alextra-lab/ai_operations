"""
Database connection utilities for the Inference Gateway using SQLAlchemy.
...
logger = configure_logging(service_name="gateway_db")
...
async def get_session() -> AsyncGenerator[AsyncSession, None]:
    ...
    try:
        ...
        logger.error(
            "Session rollback due to error: %s",
            type(e).__name__,
            exc_info=True,
        )
        raise
...
async def init_db() -> None:
    """
    Initialize the inference gateway database tables.
    ...
    """
    from . import usage  # noqa: F401 - Import to register table definitions

    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)
    logger.info("Inference Gateway database tables initialized.")
